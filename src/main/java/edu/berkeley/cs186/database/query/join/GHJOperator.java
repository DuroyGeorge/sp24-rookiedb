...
public class GHJOperator extends JoinOperator {
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() <= this.numBuffers - 2 && leftPartitions[i].getNumPages() > 0 || rightPartitions[i].getNumPages() <= this.numBuffers - 2 && rightPartitions[i].getNumPages() > 0) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // Create enough records to exceed the memory limit for SHJ
        for (int i = 0; i < 100; i++) {  // 6 pages * 8 records per page
            leftRecords.add(createRecord(i));
            rightRecords.add(createRecord(i));
        }

        for (int i = 0; i < 100; i++) {
            leftRecords.add(createRecord(i + 100));
            rightRecords.add(createRecord(i + 100));
        }
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        // Create enough records to require more recursive passes than allowed
        for (int i = 0; i < 1024; i++) {  // 128 pages * 8 records per page
            leftRecords.add(createRecord(i % 10));  // Repeated values to ensure multiple passes
