...
import edu.berkeley.cs186.database.query.join.*;
...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int seq = minOp.estimateIOCost();
        int except = -1;
        List<Integer> indexes = getEligibleIndexColumns(table);
        for (int i : indexes) {
            SelectPredicate p = this.selectPredicates.get(i);
            QueryOperator indexOp = new IndexScanOperator(this.transaction, table, p.column, p.operator, p.value);
            int index = indexOp.estimateIOCost();
            if (index < seq) {
                minOp = indexOp;
                seq = index;
                except = i;
...
//        allJoins.add(new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction));
...
//        allJoins.add(new SortMergeOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction));
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> i : prevMap.keySet()) {
            for (JoinPredicate j : this.joinPredicates) {
                String leftTable = j.leftTable;
                String rightTable = j.rightTable;
                if (i.contains(leftTable) && !i.contains(rightTable)) {
                    Set<String> temp = new HashSet<>();
                    temp.add(rightTable);
                    QueryOperator rightOp = pass1Map.get(temp);
                    QueryOperator joinOp = minCostJoinType(prevMap.get(i), rightOp, j.leftColumn, j.rightColumn);
                    Set<String> newSet = new HashSet<>(i);
                    newSet.add(rightTable);
                    if (!result.containsKey(newSet) || joinOp.estimateIOCost() < result.get(newSet).estimateIOCost()) {
                        result.put(newSet, joinOp);
                    }
                } else if (i.contains(rightTable) && !i.contains(leftTable)) {
                    Set<String> temp = new HashSet<>();
                    temp.add(leftTable);
                    QueryOperator leftOp = pass1Map.get(temp);
                    QueryOperator joinOp = minCostJoinType(prevMap.get(i), leftOp, j.rightColumn, j.leftColumn);
                    Set<String> newSet = new HashSet<>(i);
