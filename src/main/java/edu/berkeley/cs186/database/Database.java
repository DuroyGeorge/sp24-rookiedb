...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                List<Lock> tracker = new ArrayList<>();
                do {
                    tracker = lockManager.getLocks(getTransaction());
                    for (Lock lock : tracker) {
                        LockContext temp = LockContext.fromResourceName(lockManager, lock.name);
                        if (temp.getNumChildren(getTransaction()) == 0) {
                            temp.release(getTransaction());
                        }
                    }
                } while (!lockManager.getLocks(getTransaction()).isEmpty());
            } catch (Exception e) {
